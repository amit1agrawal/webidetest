sap.ui.define([
	...
	return BaseObject.extend("com.spc.fiori.model.GroupSortState", {
		 ...
		 * @alias com.spc.fiori.model.GroupSortState
		 */
		...
		/**
		 * Groups by Grt, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Grt") {
				...
				this._oViewModel.setProperty("/sortBy", "Grt");

				aSorters.push(
					new Sorter("Grt", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Vssl");
			}
